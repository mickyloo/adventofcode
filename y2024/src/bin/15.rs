...
fn main() -> Result<()> {
    ...
    fn print(grid: &Vec<Vec<char>>) {
        grid.into_iter().for_each(|row| { let foo = row.into_iter().join(""); println!("{}", foo); })
    }
    ...
    fn push(grid: &mut Vec<Vec<char>>, cell: (usize, usize), dir: char) -> (usize, usize) {
        ...
        let new_cell = ((cell.0 as i32 + delta.0) as usize, (cell.1 as i32+ delta.1) as  usize);
    ...
    fn part1<R: BufRead>(reader: R) -> Result<usize> {
        ...
        let mut grid = blocks[0].iter().map(|s| {
            s.chars().collect_vec()
        }).collect_vec();
